# -*- coding: utf-8 -*-
"""
Created on Tue Apr 26 19:30:40 2016
...
if repull == 1:
    ...
    new_user = searchUser("dhey24", False)
...
for n, d in G.nodes_iter(data = True):
    ...
    #create list of fieldnames for csv
    for k in d.keys():
...
#write csv of the graph positions
#draw the graph
plt.figure(1,figsize=(40,40))
pos = nx.spring_layout(G, iterations = 100)

#create position node attribute
nx.set_node_attributes(G, 'pos', pos)

noUnicode = re.compile(r'[^\x00-\x7F]+')
def make_unicode(input):
    if type(input) == unicode:
        try:
            #input =  input.decode('unicode-escape')
            input = noUnicode.sub('*', input)
            return input
        except:
            return input
    else:
...
with open("secondDegree_"+ new_user.username + "_+" + str(countLimit) +".csv", "wb") as outfile:
    fieldnames.extend(["id", "posX", "posY"])
    fieldnames.append("connection")
    fieldnames.append("pathOrder")
    ...
    for e in G.edges_iter(data = False):
        ...
        for n in e:
            writeDict = {}
            writeDict["id"] = n
            writeDict["connection"] = make_unicode(G.node[e[0]]["username"]) + "-" + make_unicode(G.node[e[1]]["username"])
            pathOrder += 1
            writeDict["pathOrder"] = pathOrder
            for key, value in G.node[n].iteritems():
                if key == "pos":
                    writeDict["posX"] = value[0]
                    writeDict["posY"] = value[1]
                else:
                    writeDict[key] = make_unicode(value)
            try:
                writer.writerow(writeDict)
            except:
                pprint.pprint(writeDict)
                for k, v in writeDict.iteritems():
                    print v, type(v)
                raise
